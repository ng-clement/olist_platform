...
class ValidationResult:
    def __init__(self, rule: str, dataset: str, column: str,
...
class DataQualityValidator:
    ...
    def expect_column_not_null(self, df, col, dataset, max_null_rate=0.0):
        ...
        return self._record("not_null", dataset, col, passed,
                            f"null_rate={null_rate*100:.2f}% (threshold={max_null_rate*100:.1f}%)")

    def expect_column_unique(self, df, col, dataset):
        dup_count = df[col].duplicated().sum()
        return self._record("unique", dataset, col, dup_count == 0,
                            f"duplicate_count={dup_count}")

    def expect_column_between(self, df, col, dataset, min_val=None, max_val=None):
        series = df[col].dropna()
        violations = 0
        if min_val is not None: violations += (series < min_val).sum()
        if max_val is not None: violations += (series > max_val).sum()
        return self._record("between", dataset, col, violations == 0,
    ...
    def expect_referential_integrity(self, df_child, fk_col, df_parent, pk_col,
                                     child_name, parent_name):
        orphans = set(df_child[fk_col].dropna()) - set(df_parent[pk_col].dropna())
        return self._record("referential_integrity", child_name, fk_col,
                            len(orphans) == 0,
    ...
    def get_summary(self) -> pd.DataFrame:
        return pd.DataFrame([{
            "rule": r.rule, "dataset": r.dataset, "column": r.column,
...
def run_orders_suite(v, ds):
    ...
    v.expect_column_values_in_set(orders, "order_status", "orders",
    ...
    v.expect_business_rule(orders_ts,
        "order_status != 'approved' or has_approval == True",
        "orders", "approved_needs_timestamp")
...
def run_order_items_suite(v, ds):
    ...
    v.expect_referential_integrity(items, "order_id", ds["orders"], "order_id",
                                   "order_items", "orders")
    v.expect_referential_integrity(items, "product_id", ds["products"], "product_id",
                                   "order_items", "products")
    v.expect_referential_integrity(items, "seller_id", ds["sellers"], "seller_id",
                                   "order_items", "sellers")
...
def run_payments_suite(v, ds):
    ...
    v.expect_column_values_in_set(payments, "payment_type", "payments",
        {"credit_card","boleto","voucher","debit_card","not_defined"})
    v.expect_referential_integrity(payments, "order_id", ds["orders"], "order_id",
                                   "payments", "orders")
...
def run_marketing_suite(v, ds):
    ...
    v.expect_column_not_null(mql, "origin", "mql", max_null_rate=0.01)  # ~60/8000 have no origin in source data
    v.expect_column_not_null(closed, "mql_id", "closed_deals")
    v.expect_column_not_null(closed, "seller_id", "closed_deals")
    v.expect_referential_integrity(closed, "mql_id", mql, "mql_id",
                                   "closed_deals", "mql")
    ...
    v.expect_business_rule(merged, "won_date >= first_contact_date",
