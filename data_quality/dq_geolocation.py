...
class GeoQualityChecker:
    ...
    def _result(self, category, name, passed, severity, rows_affected=0, detail=""):
        ...
        r = {
            "check_id": f"GEO-{self._check_id:02d}",
            "category": category,
            "name": name,
            "status": status,
            "severity": severity,
    ...
    def check_required_columns(self):
        ...
        self._result(
            ...
            "CRITICAL",
            len(missing),
            f"Missing: {missing}" if missing else "All 5 required columns present",
        )
    ...
    def check_column_count(self):
        self._result(
            ...
            len(self.df.columns) == 5,
            "WARNING",
            0,
            f"Expected 5, found {len(self.df.columns)}",
        )
    ...
    def check_lat_brazil(self):
        b = BRAZIL_BOUNDS
        out = (
            (self.df["geolocation_lat"] < b["lat_min"])
            | (self.df["geolocation_lat"] > b["lat_max"])
        ).sum()
        pct = out / len(self.df) * 100
        self._result(
            "Coordinate",
            "Latitude within Brazil bounding box",
            pct < 0.01,
            "WARNING",
            out,
    ...
    def check_lng_brazil(self):
        b = BRAZIL_BOUNDS
        out = (
            (self.df["geolocation_lng"] < b["lng_min"])
            | (self.df["geolocation_lng"] > b["lng_max"])
        ).sum()
        pct = out / len(self.df) * 100
        self._result(
            "Coordinate",
            "Longitude within Brazil bounding box",
            pct < 0.01,
            "WARNING",
            out,
    ...
    def check_lat_lng_not_swapped(self):
        ...
        self._result(
            "Coordinate",
            "Latitude not accidentally positive-large",
            s < 10,
            "WARNING",
            int(s),
            f"{s} rows lat>10",
    ...
    def check_state_count(self):
        ...
        self._result(
            "ReferentialIntegrity",
            "All 27 Brazilian states present",
            n >= 27,
            "INFO",
            0,
    ...
    def check_zip_range(self):
        ...
        self._result(
            "ReferentialIntegrity",
            "Zip codes within [01001–99999]",
            out == 0,
            "WARNING",
            int(out),
            f"{out} invalid zip codes",
        )
    ...
    def check_customer_zip_coverage(self):
        if self.customer_zips is None:
            self._result(
                "ReferentialIntegrity",
                "Customer zip coverage (skipped)",
                True,
                "INFO",
                0,
                "No customer zip series provided",
            )
            return
        geo_zips = set(
            pd.to_numeric(self.df["geolocation_zip_code_prefix"], errors="coerce")
        ...
        self._result(
            "ReferentialIntegrity",
            "All customer zips have geo coverage",
            pct < 5,
            "WARNING",
            len(missing),
    ...
    def check_unique_zip_count(self):
        ...
        self._result(
            "BusinessLogic",
            "Unique zip count within [15K–25K]",
            15000 <= n <= 25000,
            "WARNING",
            0,
    ...
    def check_total_row_count(self):
        ...
        self._result(
            "BusinessLogic",
            "Total row count within [900K–1.1M]",
            900_000 <= n <= 1_100_000,
            "WARNING",
            0,
    ...
    def check_dedup_ratio(self):
        if self.df_deduped is None:
            self._result(
                "BusinessLogic",
                "Dedup ratio check (skipped)",
                True,
                "INFO",
                0,
                "No deduped df provided",
            )
            return
    ...
    def check_city_name_quality(self):
        sus = (
            self.df["geolocation_city"]
            .apply(lambda x: len(str(x).strip()) < 2 or str(x).strip().isdigit())
            .sum()
        )
        pct = sus / len(self.df) * 100
        self._result(
            "BusinessLogic",
            "City names are meaningful strings",
            pct < 1.0,
            "WARNING",
            int(sus),
    ...
    def run_all(self) -> dict:
        ...
        for method in [
            ...
            self.check_lat_numeric,
            self.check_lng_numeric,
            self.check_zip_parseable,
            self.check_null_zip,
            self.check_null_lat,
            self.check_null_lng,
            self.check_null_city,
            self.check_null_state,
            self.check_lat_range_global,
            self.check_lng_range_global,
            self.check_lat_brazil,
            self.check_lng_brazil,
            self.check_zero_coordinates,
            self.check_lat_lng_not_swapped,
            self.check_coordinate_precision,
            self.check_valid_state_codes,
            self.check_state_count,
            self.check_zip_range,
            self.check_customer_zip_coverage,
            self.check_sp_is_largest_state,
            self.check_unique_zip_count,
            self.check_total_row_count,
            self.check_dedup_ratio,
            self.check_city_name_quality,
            self.check_lat_iqr_outliers,
            self.check_lng_iqr_outliers,
            self.check_lat_std,
            self.check_lng_std,
        ]:
            method()
        ...
        logger.info("─" * 60)
        logger.info(
            "SUMMARY: %d/%d passed | %d warnings | %d failures | %.1fs",
            passed,
            total,
            warned,
            failed,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source",
        default=str(DATA_DIR / "olist_geolocation_dataset.csv"),
    ...
    print(
        f"Pass rate: {summary['pass_rate']}% | Pipeline proceed: {summary['pipeline_can_proceed']}"
    )
