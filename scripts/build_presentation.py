...
def add_textbox_multi(
    slide, lines, x, y, w, h, font_size=11, color=DARK_TXT, line_spacing=1.0
):
...
def header_band(slide, title, slide_num, total=13):
    ...
    add_text(
        slide,
        "OLIST ANALYTICS PLATFORM  |  CONFIDENTIAL",
        0.18,
        0.03,
        7,
        0.22,
        font_size=7.5,
        color=rgb(0x9F, 0xD7, 0xF0),
        bold=False,
        align=PP_ALIGN.LEFT,
    )
    add_text(
...
def kpi_card(
    slide, label, value, unit, x, y, w=1.7, h=0.82, value_color=WHITE, bg=TEAL
):
...
def bullet_box(
    ...
    if bg:
        add_rect(
            slide, x, y, w, h, fill_color=bg, line_color=line_color, line_width=0.5
        )
    txb = slide.shapes.add_textbox(
        Inches(x + 0.1), Inches(y + 0.08), Inches(w - 0.18), Inches(h - 0.12)
    )
    txb.word_wrap = True
    tf = txb.text_frame
    tf.word_wrap = True
...
def two_col_table(
    slide, rows, x, y, w, col_w, font_size=9.5, header_bg=TEAL, alt_bg=LIGHT_BG
):
...
add_text(
    slide,
    "Unified analytics across 11 datasets · Star schema warehouse · 181+ data quality checks · 7 dbt mart models",
    0.4,
    2.54,
    9.2,
    0.3,
    font_size=9.5,
    color=rgb(0x8A, 0xC4, 0xE0),
    align=PP_ALIGN.CENTER,
)
...
add_text(
    slide,
    "Module 2 Assignment  |  May 2026  |  NTU SCTP Advanced Professional Certificate — Data Science & AI",
    0.3,
    5.32,
    9.4,
    0.24,
...
for ci, (title, col, items) in enumerate(col_data):
    ...
    add_text(
        slide, title, x + 0.08, 1.44, 3.0, 0.24, font_size=8.5, bold=True, color=WHITE
    )
    add_rect(
        slide,
        x,
        ...
        3.1,
        3.62,
        fill_color=rgb(0xF6, 0xFB, 0xFF)
        if ci == 0
        else rgb(0xFF, 0xF8, 0xF3)
        if ci == 1
        else rgb(0xF4, 0xFD, 0xFB),
        line_color=rgb(0xCC, 0xDD, 0xEE),
        line_width=0.5,
    )
...
for i, (title, desc) in enumerate(problems):
    y = 1.32 + i * 1.01
    add_rect(
        slide, 0.25, y, 5.35, 0.94, fill_color=LIGHT_BG, line_color=TEAL, line_width=0.8
    )
    add_text(
        slide, title, 0.38, y + 0.06, 5.1, 0.28, font_size=10.5, bold=True, color=NAVY
    )
    add_text(
        slide, desc, 0.38, y + 0.34, 5.1, 0.56, font_size=9, color=MID_TXT, wrap=True
    )

# Right: Scope & Objectives
add_text(
    slide, "PROJECT SCOPE", 5.8, 0.98, 3.9, 0.28, font_size=10.5, bold=True, color=NAVY
)
scope_items = [
...
add_rect(
    slide, 5.8, 1.30, 3.95, 1.65, fill_color=LIGHT_BG, line_color=TEAL, line_width=0.5
)
for i, (k, v) in enumerate(scope_items):
    y = 1.37 + i * 0.3
    add_text(
        slide, k.upper() + ":", 5.93, y, 1.0, 0.28, font_size=8.5, bold=True, color=TEAL
    )
...
add_text(
    slide,
    "Analytical Framework — From Raw Data to Actionable Decisions",
    0.25,
    0.98,
    9.5,
    0.32,
...
steps = [
    ...
    (
        ...
        "Why did it happen?",
        [
            "97% one-time buyer root cause",
            "Late delivery → review score impact",
            "Channel conversion gap (12% vs 3%)",
            "Regional demand vs population",
        ],
    ),
    (
        "3. PREDICT",
        ...
        "What will happen?",
        [
            "At-risk segment recovery value",
            "Paid search ROI projection",
            "Fulfilment hub demand model",
            "Subscription LTV estimate",
        ],
    ),
    (
        "4. PRESCRIBE",
        ...
        [
            ...
            "Phased roadmap Q3 2026–Q1 2027",
...
for i, (title, color, question, bullets) in enumerate(steps):
    ...
    add_text(
        slide, title, x + 0.08, 1.45, 2.18, 0.3, font_size=9.5, bold=True, color=WHITE
    )
    add_rect(slide, x, 1.78, 2.3, 0.34, fill_color=rgb(0xDD, 0xEE, 0xFF))
    add_text(
        slide, question, x + 0.08, 1.80, 2.18, 0.3, font_size=9.5, bold=True, color=NAVY
    )
    add_rect(
        slide, x, 2.12, 2.3, 1.84, fill_color=LIGHT_BG, line_color=color, line_width=0.7
    )
...
add_rect(
    ...
    9.5,
    1.22,
    fill_color=rgb(0xF0, 0xF8, 0xFF),
    line_color=TEAL,
    line_width=0.6,
)
add_text(
    slide,
    "KEY ANALYTICAL QUESTIONS ANSWERED IN THIS PRESENTATION",
    0.38,
    4.14,
    9.2,
    0.26,
...
add_text(
    slide,
    "Brazilian E-Commerce Dataset by Olist — Kaggle Public Dataset",
    0.25,
    0.98,
    9.5,
    0.3,
...
add_text(
    slide,
    "11 relational CSV files covering orders, customers, products, sellers, payments, reviews, marketing leads, and geolocation (Sep 2016 – Oct 2018).",
    0.25,
    1.28,
    9.5,
    0.28,
...
rows = [
    (
        ...
        "99,441",
        "order_id, customer_id, status, dates",
        "Core transactional grain",
    ),
    (
        "olist_order_items_dataset",
        "112,650",
        "order_id, product_id, seller_id, price",
        "Line-item revenue & freight",
    ),
    (
        "olist_customers_dataset",
        "99,441",
        "customer_id, customer_unique_id, state, zip",
        "Customer deduplication & geo",
    ),
    (
        "olist_products_dataset",
        "32,951",
        "product_id, category_name, dimensions",
        "Product classification",
    ),
    (
        "olist_sellers_dataset",
        "3,095",
        "seller_id, state, zip_code_prefix",
        "Seller geography & tier",
    ),
    (
    ...
    (
        ...
        "99,224",
        "order_id, review_score, review_dates",
        "Customer satisfaction score",
    ),
    (
        "olist_marketing_qualified_leads",
        "8,000",
        "mql_id, origin (channel), first_contact_date",
        "B2B seller acquisition funnel",
    ),
    (
        "olist_closed_deals_dataset",
        "842",
        "mql_id, seller_id, won_date, segment",
        "Conversion & deal metadata",
    ),
    (
        "olist_geolocation_dataset",
        "1,000,163",
        "zip_prefix, lat, lng, city, state",
        "Geographic intelligence (→19K)",
    ),
    (
...
layers = [
    (
        TEAL,
        "1 · SOURCE",
        "11 CSV files (data/raw/)  |  1.46M total rows  |  Python ingest scripts",
    ),
    (
        rgb(0xF4, 0xA2, 0x61),
        "2 · RAW LAYER",
        "BigQuery olist_raw  |  WRITE_TRUNCATE  |  _ingested_at + _source_file metadata",
    ),
    (
        rgb(0x28, 0x9A, 0xC4),
        "3 · STAGING",
        "dbt staging models (views)  |  Type casts · NULL filters · Deduplication",
    ),
    (
        rgb(0x1A, 0x6B, 0x4A),
        "4 · WAREHOUSE",
        "Star schema  |  4 facts + 7 dims  |  FARM_FINGERPRINT surrogate keys",
    ),
    (
        rgb(0x7B, 0x35, 0xA2),
        "5 · MARTS",
        "7 dbt mart models  |  Pre-aggregated KPIs  |  Business logic gold layer",
    ),
]
for i, (col, label, desc) in enumerate(layers):
    ...
    add_text(
        slide, label, 0.33, y + 0.12, 1.38, 0.32, font_size=8.5, bold=True, color=WHITE
    )
    add_rect(
        slide, 1.75, y, 3.75, 0.56, fill_color=LIGHT_BG, line_color=col, line_width=0.6
    )
    add_text(
        slide,
        desc,
...
add_text(
    slide, "TECH STACK: " + tech, 0.38, 4.82, 5.08, 0.36, font_size=8.5, color=NAVY
)

# Right: Data Quality pyramid
add_text(
    slide,
    "4-Layer Data Quality Framework",
    5.7,
    0.98,
    4.0,
    0.3,
...
dq_layers = [
    (
        rgb(0x7B, 0x35, 0xA2),
        "Layer 4: SQL Integration Tests",
        "6 cross-table assertions  |  Pass: 100%",
    ),
    (
        NAVY,
        "Layer 3: dbt Mart Schema Tests",
        "40+ column tests across 7 marts  |  Pass: 100%",
    ),
    (
        TEAL,
        "Layer 2: dbt Staging Schema Tests",
        "35+ tests on 9 staging models  |  Pass: 100%",
    ),
    (
        ORANGE,
        "Layer 1: Python Pre-Load DQ",
        "87 checks: 57 core + 30 geo  |  Pass: 100%",
    ),
]
...
add_text(
    slide,
    "Platform-Wide KPI Scorecard — 2016–2018 Olist Dataset",
    0.25,
    0.98,
    9.5,
    0.28,
...
add_text(
    slide,
    "Monthly GMV & Order Volume — 2016–2018",
    0.25,
    0.98,
    5.8,
    0.26,
...
add_text(
    slide,
    "Segment Analysis & Commercial Implication",
    5.95,
    0.98,
    3.8,
    0.26,
...
segments = [
    (
        ...
        "CHAMPIONS",
        "5,974 (6.2%)",
        "35% of total GMV — protect with VIP programme",
    ),
    ...
    (
        ...
        "NEW CUSTOMERS",
        "11,684 (12.2%)",
        "First-time buyers: 7-day re-engagement window",
    ),
    (
        RED,
        "CANT LOSE THEM",
        "~2,800 (2.9%)",
        "High-frequency buyers gone silent — highest urgency",
    ),
    (
        rgb(0x7B, 0x35, 0xA2),
        "SLEEPING GIANTS",
        "~4,500 (4.7%)",
        "High-spend, long recency — premium re-engagement",
    ),
    (
        DIM_TXT,
        "HIBERNATING",
        "~48,000 (50%+)",
        "Long-inactive — low-cost broadcast campaigns only",
    ),
]
for i, (col, seg, count, action) in enumerate(segments):
    ...
    add_text(
        slide, seg, 6.18, y + 0.04, 1.6, 0.22, font_size=8.5, bold=True, color=DARK_TXT
    )
...
add_text(
    slide,
    "Seller Acquisition Funnel by Channel",
    0.25,
    0.98,
    4.8,
    0.26,
...
# Funnel insights
add_rect(
    slide, 0.22, 3.92, 4.88, 1.40, fill_color=LIGHT_BG, line_color=TEAL, line_width=0.5
)
add_text(
    slide,
...
add_text(
    slide,
    "Geographic Demand Intelligence — 27 States",
    5.25,
    0.98,
    4.55,
    0.26,
...
# Geo insights
add_rect(
    slide, 5.22, 3.92, 4.58, 1.40, fill_color=LIGHT_BG, line_color=TEAL, line_width=0.5
)
add_text(
    slide,
    "REGIONAL DEMAND vs POPULATION",
...
for i, (region, pop, orders, col, note) in enumerate(geo_data):
    ...
    add_text(
        slide,
        f"{region}:  pop {pop}  |  orders {orders}  — {note}",
        5.55,
        y + 0.02,
        4.18,
        0.22,
...
add_text(
    slide,
    "Delivery Performance & Review Score Correlation",
    0.25,
    0.98,
    5.5,
    0.26,
...
# Insights row
add_rect(
    slide, 0.25, 4.30, 9.5, 1.05, fill_color=LIGHT_BG, line_color=TEAL, line_width=0.5
)
add_text(
    slide,
    "LOGISTICS & PAYMENT FINDINGS",
    0.38,
    4.34,
    9.2,
    0.24,
...
# Left: what was built
add_text(
    slide, "What Was Built", 0.25, 0.98, 4.6, 0.28, font_size=11, bold=True, color=NAVY
)
built_items = [
...
add_rect(
    slide, 0.25, 1.30, 4.6, 3.60, fill_color=LIGHT_BG, line_color=TEAL, line_width=0.5
)
...
for i, (col, title, text) in enumerate(implications):
    ...
    add_text(
        slide, text, 5.32, y + 0.28, 4.3, 0.46, font_size=8.5, color=DARK_TXT, wrap=True
    )
...
phases = [
    (
        "Q3 2026 — Foundation Live",
        "Real-time Kafka ingestion · ML churn model · Looker Studio dashboards",
    ),
    (
        "Q4 2026 — Scale",
        "Personalisation engine · Dynamic pricing · North hub · Subscription",
    ),
    (
        "Q1 2027 — Intelligence",
        "Multi-country · AI recommendation API · Self-serve analytics portal",
    ),
]
for i, (phase, detail) in enumerate(phases):
    ...
    add_text(
        slide, detail, x, 5.20, 3.1, 0.18, font_size=7, color=rgb(0x88, 0xAA, 0xCC)
    )
...
print(
    f"    Dimensions: {prs.slide_width.inches:.1f}in × {prs.slide_height.inches:.1f}in"
)
