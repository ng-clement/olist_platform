...
fig.suptitle(
    "Monthly Revenue & Order Volume — Olist 2016–2018",
    fontsize=14,
    fontweight="bold",
    y=0.98,
)
...
ch_stats = (
    ...
    .agg(leads=("mql_id", "count"), converted=("converted", "sum"))
    .reset_index()
)
...
ax.axvline(
    ch_stats["conversion_rate"].mean(),
    color="orange",
    linestyle="--",
    alpha=0.8,
    label=f"Avg {ch_stats['conversion_rate'].mean():.1f}%",
)
ax.legend()
plt.tight_layout()
...
ax1.hist(
    delivered_orders["delivery_days"].clip(0, 60),
    bins=40,
    color="#1C7293",
    alpha=0.85,
    edgecolor="white",
...
ax1.axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Median: {delivered_orders['delivery_days'].median():.0f} days",
)
...
state_late = (
    delivered_orders.groupby("customer_state")["is_late"]
    .mean()
...
state_orders = (
    ...
    .agg(order_count=("order_id", "count"), avg_value=("total_value", "mean"))
    .reset_index()
...
pay_type = (
    ...
    .agg(count=("order_id", "count"), value=("payment_value", "sum"))
    .reset_index()
)
pay_type = pay_type.sort_values("count", ascending=False)

install_dist = (
    payments[payments["payment_type"] == "credit_card"]["payment_installments"]
    .value_counts()
    .sort_index()
)
...
ax1.pie(
    ...
    autopct="%1.1f%%",
    colors=["#065A82", "#1C7293", "#4EAFD1", "#9FD7F0"],
)
